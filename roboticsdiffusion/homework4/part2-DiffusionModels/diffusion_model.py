        ...
        ** Use DL_random() to generate any random numbers **
        ...
        for i in range(len(data)):
          if DL_random(shape = (1,), normal = False, seed = seed).item() <= self.p_uncond:
            cond[i] = torch.zeros(cond[i].shape)
        ...
        ########################################
        noise = DL_random(shape = data.shape, seed = seed).to(self.device)
        
        t = DL_random(shape = (data.shape[0],), int_range = [1,self.denoising_steps+1], normal = False, seed = seed).to(self.device)
        
        t = t - torch.ones((data.shape[0],), dtype = int).to(self.device)
        noisy_data = self.noise_scheduler.add_noise(data, noise, t).to(self.device)
        pred = self.noise_pred_net(noisy_data, torch.tensor(t).to(self.device), cond).to(self.device)
        loss = noise - pred
        loss = loss**2
    ...
    def generate_sample(self, cond, guidance_weight = 0.0, threshold = False, seed = None):
        ...
        ** Use DL_random() to generate any random numbers **
        ...
        with torch.no_grad():
            ...
            xt = DL_random(shape = (cond.shape[0], *self.input_shape), seed = seed).to(self.device)
            
            uncond = torch.zeros(cond.shape, dtype = int).to(self.device)
            for t in range(self.denoising_steps-1, -1, -1):
              noisepred = (1+guidance_weight)*noise_pred_net(xt, torch.tensor(t).to(self.device), cond) - guidance_weight*noise_pred_net(xt, torch.tensor(t).to(self.device), uncond)
              xt = self.noise_scheduler.denoise_step(noisepred, t, xt, seed = seed).to(self.device)
            sample = xt
            ####################################################################################
            #                                 END OF YOUR CODE                                 #
            ####################################################################################
