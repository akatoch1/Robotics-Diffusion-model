...
def generator(noise_dim=NOISE_DIM, seed=None):
    ...
    model = nn.Sequential(
          ...
          nn.ReLU(),
          nn.ConvTranspose2d(in_channels=128, out_channels=3, kernel_size=2, stride=2, padding = 1),
          nn.Tanh()
        )
    ##############################################################################
    #                               END OF YOUR CODE                             #
...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss = None
    dtype = torch.cuda.FloatTensor if torch.cuda.is_available() else torch.FloatTensor
    real = torch.ones(logits_real.shape).type(dtype)
    fakes = torch.zeros(logits_fake.shape).type(dtype)
    ##############################################################################
    # TODO: Implement Disc. Loss                                                 #
    ##############################################################################
    real_loss = bce_loss(logits_real, real)
    fake_loss = bce_loss(logits_fake, fakes)
...
def generator_loss(logits_fake):
    ...
    ##############################################################################
    reals = torch.ones(logits_fake.shape).type(dtype)
    loss = bce_loss(logits_fake, reals)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return loss
    ...
    for epoch in range(num_epochs):
        for x, _ in loader_train:
           ...
           # print('hi2')
            ...
            fake_output = D(fake_images).type(dtype)
           # print('hi3')
            d_total_error = discriminator_loss(logits_real, fake_output).type(dtype)
           # print('hi4')
